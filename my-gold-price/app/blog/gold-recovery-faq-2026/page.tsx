import { Metadata } from 'next';
import Image from 'next/image';

export const metadata: Metadata = {
  title: '黃金回收FAQ｜賣黃金常見問題一篇搞懂',
  description: '2026年黃金回收常見問題集！最新金價行情、估價流程、證件要求，一篇搞懂所有疑問，讓你放心到台北巧品珠寶賣黃金。 台北巧品珠寶鑑定師L親自鑑定，10年以上珠寶鑑定資歷，透明報價不扣耗損，台北大安區濟南路三段62-1號，提供免費估價與諮詢服務。',
  keywords: '黃金回收 FAQ,黃金回收問題,賣黃金問題',
  alternates: {
    canonical: 'https://www.gold-tw.com/blog/gold-recovery-faq-2026',
  },
  openGraph: {
    locale: 'zh_TW',
    title: '黃金回收FAQ｜賣黃金常見問題一篇搞懂',
    description: '2026年黃金回收常見問題集！最新金價行情、估價流程、證件要求，一篇搞懂所有疑問，讓你放心到台北巧品珠寶賣黃金。',
    type: 'article',
    url: 'https://www.gold-tw.com/blog/gold-recovery-faq-2026',
    siteName: '巧品珠寶',
    publishedTime: '2026-03-01T00:00:00+08:00',
    modifiedTime: '2026-03-31T00:00:00+08:00',
    images: [{ url: 'https://res.cloudinary.com/dakafmdhy/image/upload/v1775483936/gold-tw/gold-recovery-faq-2026-cover.png', width: 1200, height: 630, alt: '2026黃金回收FAQ｜專業鑑定師回答常見回收問題' }],

  },
};

// lastModified: 2026-03-19
export default function Page() {
  return (
    <main style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto' }}>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            '@context': 'https://schema.org',
            '@type': 'BlogPosting',
            headline: '黃金回收FAQ｜賣黃金常見問題一篇搞懂',
            description: '2026年黃金回收常見問題集！最新金價行情、估價流程、證件要求，一篇搞懂所有疑問，讓你放心到台北巧品珠寶賣黃金。',
            mainEntityOfPage: { '@type': 'WebPage', '@id': 'https://www.gold-tw.com/blog/gold-recovery-faq-2026' },
            author: { '@type': 'Person', name: '鑑定師L', url: 'https://www.gold-tw.com' },
            publisher: { '@type': 'Organization', name: '巧品珠寶', url: 'https://www.gold-tw.com' },
            datePublished: '2026-03-01T00:00:00+08:00',
            dateModified: '2026-03-31T00:00:00+08:00',
          })
        }}
      />
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            "@context": "https://schema.org",
            "@type": "FAQPage",
            "headline": "黃金回收FAQ｜賣黃金常見問題一篇搞懂",
            "author": { "@type": "Person", "name": "鑑定師L", "url": "https://www.gold-tw.com", "@id": "https://www.gold-tw.com/#business" },
            "publisher": { "@type": "Organization", "name": "巧品珠寶", "url": "https://www.gold-tw.com", "logo": { "@type": "ImageObject", "url": "https://wsrv.nl/?url=https://drive.google.com/uc?id=16UrCpYWRkyEkvL7M73o0DIAvkKWtF0Lp&w=200&output=webp" } },
            "datePublished": "2026-03-14",
              "dateModified": "2026-03-16T00:00:00+08:00",
            "description": "黃金回收常見問題大集合！秤重、扣重、證件、流程一篇搞懂",
            "mainEntity": [
              {"@type":"Question","name":"黃金秤重用什麼單位？","acceptedAnswer":{"@type":"Answer","text":"台灣常用「錢」為單位，1錢=3.75克。我們也提供公克單位讓你選擇。"}},
              {"@type":"Question","name":"秤重時要注意什麼？","acceptedAnswer":{"@type":"Answer","text":"一定要親眼看見秤重數字，確保過程公開透明。如果店家不讓你看秤，這是警示信號，建議換店。"}},
              {"@type":"Question","name":"自己在家秤的重量跟店裡不同？","acceptedAnswer":{"@type":"Answer","text":"不同秤可能有誤差，建議以專業珠寶秤為準。一般廚房秤誤差可能達0.5-1克，珠寶秤精確度更高。"}},
              {"@type":"Question","name":"台灣黃金回收「一錢」是多少錢？","acceptedAnswer":{"@type":"Answer","text":"1錢（3.75克）的9999純金，依當日回收價計算。2026年黃金價格約每克2,000-2,500台幣，即每錢約7,500-9,375元。實際以當日牌價為準。"}},
              {"@type":"Question","name":"賣黃金要帶什麼證件？","acceptedAnswer":{"@type":"Answer","text":"需攜帶身份證+健保卡進行實名制登記，這是政府法規要求。"}},
              {"@type":"Question","name":"沒有身份證可以賣嗎？","acceptedAnswer":{"@type":"Answer","text":"不行，這是政府規定的實名制要求，必須有身份證正本。未成年需法定代理人陪同。"}},
              {"@type":"Question","name":"什麼是扣耗損？","acceptedAnswer":{"@type":"Answer","text":"有些店家會扣除5-10%作為耗損費。巧品珠寶不扣任何耗損，按實際重量計算。"}},
              {"@type":"Question","name":"多久能拿到錢？","acceptedAnswer":{"@type":"Answer","text":"巧品珠寶現場立即現金付款，不用等1-2天。確認交易後立刻數鈔票給您，全程安全透明。"}},
              {"@type":"Question","name":"整個流程要多久？","acceptedAnswer":{"@type":"Answer","text":"一般約10-15分鐘，包含秤重+儀器檢測+確認+付款。"}},
              {"@type":"Question","name":"需要預約嗎？","acceptedAnswer":{"@type":"Answer","text":"建議預約以避免久候，但也可以直接過來。LINE預約最方便：@QPD888。"}}
            ]
          })
        }}
      />
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"首頁","item":"https://www.gold-tw.com"},{"@type":"ListItem","position":2,"name":"知識專欄","item":"https://www.gold-tw.com/blog"},{"@type":"ListItem","position":3,"name":"黃金回收FAQ｜賣黃金常見問題一篇搞懂"}]})
        }}
      />
      <h1>黃金回收FAQ｜賣黃金常見問題一篇搞懂</h1>

      <div style={{ margin: '1.5rem 0', borderRadius: '12px', overflow: 'hidden' }}>
        <Image
          src="https://res.cloudinary.com/dakafmdhy/image/upload/v1775483936/gold-tw/gold-recovery-faq-2026-cover.png"
          alt="2026黃金回收FAQ｜專業鑑定師回答常見回收問題"
          width={1200}
          height={630}
          style={{ width: '100%', height: 'auto' }}
          priority
        />
      </div>

      <div style={{background:'#fffbeb', borderLeft:'4px solid #f59e0b', padding:'16px', margin:'16px 0', borderRadius:'8px'}}>
        <strong>📌 重點摘要：</strong> 2026年黃金回收最新FAQ：目前回收價行情、需要什麼證件、K金怎麼計價、哪裡回收最划算，一次解答。
      </div>

      <div style={{ background: '#fff3f3', border: '1px solid #fca5a5', borderRadius: '8px', padding: '16px 20px', margin: '24px 0' }}>
        <strong style={{ color: '#dc2626' }}>這篇不適合你，如果…</strong>
        <ul style={{ margin: '8px 0 0', paddingLeft: '20px', color: '#374151' }}>
        <li>你只想快速知道今天金價 → <a href="/daily-gold-price" style={{ color: '#b45309' }}>直接看今日金價</a></li>
        <li>你的黃金是K金（非純金）→ <a href="/blog/karat-gold-guide" style={{ color: '#b45309' }}>看K金回收說明</a></li>
        <li>你想了解鑽石回收行情 → <a href="/blog/diamond-recovery-price" style={{ color: '#b45309' }}>鑽石回收說明</a></li>
        </ul>
      </div>


      


      <h2>結論：第一次賣黃金？所有常見問題這篇都有答案！</h2>
      <p>2026年黃金價格持續維持在歷史高位，是賣黃金的好時機。無論您有多少黃金首飾、金條、舊金需要變現，本篇FAQ將解答所有疑問，讓您安心到巧品珠寶完成交易。更多金價資訊可參考<a href="/blog/gold-price-2026" style={{ color: '#B8860B' }}>2026年黃金行情</a>。</p>

      <h2>秤重有哪些常見問題？</h2>
      <h3>Q：黃金秤重用什麼單位？</h3>
      <p>A：台灣常用「錢」為單位，1錢=3.75克。我們也提供公克單位讓你選擇。了解更多<a href="/blog/gold-weight" style={{ color: '#B8860B' }}>黃金重量換算</a>方式。</p>

      <h3>Q：秤重時要注意什麼？</h3>
      <p>A：一定要<strong>親眼看見秤重數字</strong>，確保過程公開透明。如果店家不讓你看秤，這是警示信號，建議換店。</p>

      <h3>Q：自己在家秤的重量跟店裡不同？</h3>
      <p>A：不同秤可能有誤差，建議以我們的專業珠寶秤為準。一般廚房秤誤差可能達0.5-1克，珠寶秤精確度更高。</p>

      <h3>Q：台灣黃金回收「一錢」是多少錢？</h3>
      <p>A：1錢（3.75克）的9999純金，依當日回收價計算。2026年黃金價格約每克2,000-2,500台幣，即每錢約7,500-9,375元。實際以當日牌價為準，可查看<a href="/today-gold-price" style={{ color: '#B8860B' }}>今日黃金回收價</a>。</p>

      <h2>證件有哪些常見問題？</h2>
      <h3>Q：賣黃金要帶什麼證件？</h3>
      <p>A：需攜帶<strong>身份證+健保卡</strong>進行實名制登記，這是政府法規要求。完整說明見<a href="/blog/gold-id-requirement" style={{ color: '#B8860B' }}>黃金回收證件需求</a>。</p>

      <h3>Q：沒有身份證可以賣嗎？</h3>
      <p>A：不行，這是政府規定的實名制要求，必須有身份證正本。未成年需法定代理人陪同。</p>

      <h3>Q：代替他人出售黃金需要什麼文件？</h3>
      <p>A：需要本人身份證及授權委託書，或黃金所有人到場。建議事先聯繫確認所需文件。</p>

      <h2>扣重有哪些常見問題？</h2>
      <h3>Q：什麼是扣耗損？</h3>
      <p>A：有些店家會扣除5-10{'%'}作為耗損費。我們<strong>不扣任何耗損</strong>，按實際重量計算。扣耗損是不誠信行為，詳見<a href="/blog/gold-recycling-traps" style={{ color: '#B8860B' }}>黃金回收常見陷阱</a>。</p>

      <h3>Q：扣重合理嗎？</h3>
      <p>A：一般合理扣重為1-3%（用於計算鑑定損耗），超過5%就太高了。巧品珠寶完全不扣重，讓您拿到最高回收金額。</p>

      <h2>付款有哪些常見問題？</h2>
      <h3>Q：多久能拿到錢？</h3>
      <p>A：我們<strong>現場立即現金</strong>付款，不用等1-2天。確認交易後立刻數鈔票給您，全程安全透明。</p>

      <h3>Q：可以匯款嗎？</h3>
      <p>A：可以，我們也提供匯款服務，確認帳號後立即匯款。部分客戶因攜帶大量現金不方便，選擇匯款方式。</p>

      <h3>Q：有沒有交易稅？</h3>
      <p>A：個人出售黃金一般不需繳納交易稅，但若涉及鉅額或商業性質交易，建議諮詢會計師。了解更多<a href="/blog/gold-tax" style={{ color: '#B8860B' }}>黃金交易稅務說明</a>。</p>

      <h2>流程有哪些常見問題？</h2>
      <h3>Q：整個流程要多久？</h3>
      <p>A：一般約10-15分鐘，包含秤重+儀器檢測+確認+付款。詳細步驟見<a href="/blog/gold-recycling-process" style={{ color: '#B8860B' }}>黃金回收完整流程</a>。</p>

      <h3>Q：需要預約嗎？</h3>
      <p>A：建議預約以避免久候，但也可以直接過來。LINE預約最方便：@QPD888。</p>

      <h3>Q：第一次賣黃金要注意什麼？</h3>
      <p>A：請先閱讀<a href="/blog/before-selling-gold" style={{ color: '#B8860B' }}>賣黃金前必知事項</a>，了解計價方式、如何避免詐騙、需要攜帶什麼等重要資訊。</p>

      <h2>特殊黃金類型有哪些問題？</h2>
      <h3>Q：繼承的黃金可以直接賣嗎？</h3>
      <p>A：可以，攜帶本人身份證到店即可。詳見<a href="/blog/inheritance-gold" style={{ color: '#B8860B' }}>遺產黃金回收說明</a>。</p>

      <h3>Q：古早金飾（祖傳金）可以回收嗎？</h3>
      <p>A：當然可以，詳見<a href="/blog/gold-antique" style={{ color: '#B8860B' }}>古早黃金回收</a>說明。早期台灣金飾純度往往很高，回收價值不低。</p>

      <div style={{ margin: '2rem 0', padding: '1.5rem', background: '#FEF9E7', borderRadius: '8px', borderLeft: '4px solid #B8860B' }}>
        <strong>延伸閱讀：</strong>
        <ul style={{ marginTop: '0.5rem', paddingLeft: '1.5rem' }}>
          <li><a href="/blog/gold-recovery-faq" style={{ color: '#B8860B' }}>黃金回收FAQ</a></li>
          <li><a href="/blog/gold-appraise" style={{ color: '#B8860B' }}>黃金免費估價</a></li>
          <li><a href="/blog/gold-recycling-process" style={{ color: '#B8860B' }}>黃金回收流程</a></li>
          <li><a href="/blog/what-to-bring-when-selling-gold-2026" style={{ color: '#B8860B' }}>賣黃金要帶什麼</a></li>
          <li><a href="/blog/before-selling-gold" style={{ color: '#B8860B' }}>賣黃金前必知事項</a></li>
          <li><a href="/blog/gold-scam-prevent" style={{ color: '#B8860B' }}>黃金詐騙防範</a></li>
        </ul>
      </div>

      <hr style={{ margin: '2rem 0' }} />

      <h2>📞 聯繫我們</h2>
      <p>📞 電話：<a href="tel:0986821626" style={{ color: '#B8860B' }}>0986-821-626</a></p>
      <p>💬 LINE：<a href="https://line.me/ti/p/~@QPD888" target="_blank" style={{ color: '#06C755' }} rel="noopener noreferrer">@QPD888</a></p>
      <p>🌐 官網：<a href="https://www.gold-tw.com/" style={{ color: '#B8860B' }} target="_blank" rel="noopener noreferrer">www.gold-tw.com</a></p>
      <p>📍 地址：台北市大安區濟南路三段62-1號</p>
    </main>
  );
}
